...
def callCrema(crema_dir, audioInFile, name, crema_ext = 'lab'):
    ...
    if not os.path.exists(crema_dir):
        os.mkdir(crema_dir)
    utils_path = os.path.dirname(os.path.realpath(__file__))
    cremaFile = os.path.join(crema_dir, name + "." + crema_ext)
    jam = analyze(filename=audioInFile)
    intervals, values = jam.annotations[0].to_interval_values()
    with open(cremaFile, 'w') as content_file:
        for i in range(len(values)):
            content_file.write(str(intervals[i][0]) + '\t' + str(intervals[i][1]) + '\t' + values[i] + '\n')
...
def callMusOOEvaluatorForEachFileInList(
        fileList, refdir, testdir, durationMap, chords = '5JazzFunctions'):
    resultByFile = {}
    ...
    for key, value in resultByFile.items():
        ...
        totalTruth += accuracyByFile[key] * durationMap[key]
        totalDuration += durationMap[key]
    return accuracyByFile, totalTruth / totalDuration
...
def evaluateFileList(
        ...
        predictionFunction =callChordino):
    durationMap = loadDurationMap(jsonList)
...
def testCV(
    ...
    durationMap = loadDurationMap(ALL_FILES)
...
#validation.trainGMM_CV(
#    trainingChromaParams = ll.ChromaEvaluationParameters(2048, 1.2),
#    gmmParams=chordModel.BasicGMMParameters(norm=None, preprocessing=None, nComponents=np.array([20,  16,  25,   4,   4]), covarianceTypes=np.repeat('full', chordModel.N_CHORD_KINDS)))
#36.189%
#36.40
#validation.trainGMM_CV(
#    trainingChromaParams = ll.ChromaEvaluationParameters(2048, 1.2),
#    gmmParams=chordModel.BasicGMMParameters(norm=None, preprocessing=None, nComponents=np.array([25, 25, 25, 10, 10]), covarianceTypes=np.repeat('full', chordModel.N_CHORD_KINDS)))

#36.588084526210636%
#validation.trainGMM_CV(
#    trainingChromaParams = ll.ChromaEvaluationParameters(2048, 1.2),
#    gmmParams=chordModel.BasicGMMParameters(norm=None, preprocessing=None, nComponents=np.array([25, 25, 25, 10, 10]), covarianceTypes=np.repeat('full', chordModel.N_CHORD_KINDS)))
#res = validation.testCV(testingChromaParams = ll.ChromaEvaluationParameters(2048, 0.9))

# 36.087334814182576% WTF !!!!!!!!!!
#validation.trainGMM_CV(
#    trainingChromaParams = ll.ChromaEvaluationParameters(2048, 1.2),
#    gmmParams=chordModel.BasicGMMParameters(norm=None, preprocessing=None, nComponents=np.array([20, 20, 20, 3, 5]), covarianceTypes=np.repeat('full', chordModel.N_CHORD_KINDS)))
...
# with old annotations: 36.65429660330279%
#  with old annotations, anotehr seed(18):36.65709822114321%
#36.82467044893764%
#validation.trainGMM_CV(
#    trainingChromaParams = ll.ChromaEvaluationParameters(2048, 0.9),
#    gmmParams=chordModel.BasicGMMParameters(norm=None, preprocessing=None, nComponents=np.array([20, 20, 20, 3, 5]), covarianceTypes=np.repeat('full', chordModel.N_CHORD_KINDS)))
#res = validation.testCV(testingChromaParams = ll.ChromaEvaluationParameters(2048, 0.9))

# 33.504880861064954%
#validation.trainGMM_CV(
#    trainingChromaParams = ll.ChromaEvaluationParameters(2048, 2.75),
#    gmmParams=chordModel.BasicGMMParameters(norm=None, preprocessing=None, nComponents=np.array([20, 20, 20, 3, 5]), covarianceTypes=np.repeat('full', chordModel.N_CHORD_KINDS)))
#res = validation.testCV(testingChromaParams = ll.ChromaEvaluationParameters(2048, 2.75))
# try 2aa0846476988a4e80259e94b9bbb2c2813fd447

# 35.41556099937098%
#validation.trainGMM_CV(
#    trainingChromaParams = ll.ChromaEvaluationParameters(2048, 0.65),
#    gmmParams=chordModel.BasicGMMParameters(norm=None, preprocessing=None, nComponents=np.array([20, 20, 20, 3, 5]), covarianceTypes=np.repeat('full', chordModel.N_CHORD_KINDS)))
#res = validation.testCV(testingChromaParams = ll.ChromaEvaluationParameters(2048, 0.65))


#validation.trainGMM_CV(
#    trainingChromaParams = ll.ChromaEvaluationParameters(2048, 0.9),
#    gmmParams=chordModel.BasicGMMParameters(norm=None, preprocessing=None, nComponents=np.array([20, 20, 20, 3, 5]), covarianceTypes=np.repeat('full', chordModel.N_CHORD_KINDS)))
#res = validation.testCV(testingChromaParams = ll.ChromaEvaluationParameters(2048, 1.25))

symStat, topTwoGrams, topNGrams, transitionMatrix = symbolicAnalysis.estimateStatistics(commonUtils.loadFileList('old.txt'), 10, 10)
symbolicAnalysis.saveTransitionMatrix('matrix.npz', transitionMatrix)