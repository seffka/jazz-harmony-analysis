...
def updateNGrams(segments, twoGrams, nGrams, limit):
    # ignore unclassified.
    kinds = list(map(lambda x: x.kind, filter(lambda x: x.kind != ll.UNCLASSIFIED, segments)))
    roots = list(map(lambda x: x.root, filter(lambda x: x.kind != ll.UNCLASSIFIED, segments)))
...
def makeTransitionCRootPart(twoGrams, harmonicRhythm):
    ...
    for i in range(chordModel.N_CHORD_KINDS):
        ...
        relatedKeys = list(filter(lambda x: x.endswith(sym), twoGrams.keys()))
...
def harmonicRhythmForFile(annoFileName):
   with open(annoFileName) as json_file:
       ...
       # remove unclassified.
       nBeats = list(map(lambda x: x.nBeats, filter(lambda x: x.kind!= ll.UNCLASSIFIED, segments)))
...
def estimateStatistics(fileList, top = 300, maxNGram = 2000):
    ...
    for infile in fileList:
        ...
        with open(infile) as json_file:
            ...
            allLabels = np.append(allLabels, list(map(lambda x: x.labels, segments)))
            allKinds = np.append(allKinds, list(map(lambda x: x.kind, segments)))
            allRoots = np.append(allRoots, list(map(lambda x: x.root, segments)))
            allDurations = np.append(allDurations, list(map(lambda x: x.duration, segments)))
            allNBeats = np.append(allNBeats, list(map(lambda x: x.nBeats, segments)))
